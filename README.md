# 🚗 CARZX-Showroom-Website

CarX Showroom Website is a modern, fully responsive car showcase website created using HTML, CSS, and Bootstrap.
This project represents a mock car showroom brand where users can explore premium cars, browse different brands, and view collections from the featured brand.
It is built to demonstrate strong front-end development skills, clean UI design, and responsive layout techniques.

---
## 🚀 Features

- Clean and modern UI
- Fully responsive design (mobile, tablet, desktop) 
- Premium and Exclusive car collection section
- Brand-wise car exploration  
- Smooth navigation and structured layout
- Built completely using HTML, CSS, and Bootstrap
- No backend — purely front-end based showcase website
---

## 🛠️ Tech Stack Used

- **HTML**  
- **CSS**   
- **Bootstrap** 
---
## 📸 Project Screenshots
![Preview](cpreview/Screenshot%202025-11-27%20020612.png)
![Preview](cpreview/Screenshot%202025-11-27%20021203.png)
![Preview](cpreview/Screenshot%202025-11-27%20021221.png)
![Preview](cpreview/Screenshot%202025-11-27%20021237.png)
![Preview](cpreview/Screenshot%202025-11-27%20021255.png)

👨‍💻 Author

Om VInod Nandre

GitHub: https://github.com/omnandre07

LinkedIn: https://www.linkedin.com/in/om-nandre-0b8569397/
